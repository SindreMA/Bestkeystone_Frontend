  ...
  <div id="LookupMain" class="HeaderFont">
    ...
    >
     ...
     <template v-if="characterData.characters.length > 0" class="flex justify-center">
      ...
      <br>
      <keystoneFriends :region="characterData.characters[0].region" :runs="filterdRuns" :mainPlayerId="characterData.characters[0].player_id"/>
      <br>
      <renameBox v-if="characterData.characters[0].renames && characterData.characters[0].renames.length != 0" :renames="characterData.characters[0].renames"></renameBox>        
      <br v-if="characterData.characters[0].renames && characterData.characters[0].renames.length != 0">
...
import Generalinfo from "../components/Pages/lookup/new_GeneralInfo.vue";
      ...
      await axios
        .get(
          `${apiUrl}/Player/new_details?character=${
            character.name
          }&realm=${character.realm}&region=${character.region}&checkblizzard=false`
        )
        .then(x => {
          if (x.data) {
