    ...
    <div class="kc-container">
      ...
      <KcCard v-else-if="error" :level="1">
        <div class="kc-plan__empty">
          <div class="kc-plan__empty-icon">⚠</div>
          Couldn't load farm recommendations right now.
        </div>
      </KcCard>

      <!-- empty (endpoint returns [] until the aggregate job has run) -->
      <KcCard v-else :level="1">
        <div class="kc-plan__empty">
          <div class="kc-plan__empty-icon">⛏️</div>
          No farm recommendations yet — check back once the season has more runs.
        </div>
      </KcCard>
    </div>
  </div>
...
/* ------------------------------------------------------------------
   ...
   GET {apiUrl}/Meta/planner?periode=
   -> PlannerRow[] = [{ zone, cur, next, gain, attain }]  (shape matches
   the former mock contract; MetaController was built to the same one).
...
function fetchPlanner() {
  ...
  const url = `${data.apiUrl}/Meta/planner?periode=${periode}`
  axios
    .get(url)
    .then((r) => {
      source.value = Array.isArray(r.data) ? r.data : []
      loading.value = false
    })
    .catch((e) => {
      console.log(e)
...
watch(() => data.SelectedPeriode, fetchPlanner)
